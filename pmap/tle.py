...
def check_tle_data(quiet):
    ...
    if(age > tle_lifetime):
        if(auto_update_tle):
            if(not quiet): print("TLE data is", age.days, "day(s) old\nAutomatically updating TLE data")
            update_tle_data(quiet)
            return True
        if(notify_old_tle):
            if(not quiet): print("Warning: TLE data is", (age - tle_lifetime).days, "days out of date")
            return False
    else:
        return True